...
class MB_Include_Exclude
{
	...
	public static function maybe_exclude_include( $type, $meta_box )
	{
		...
		foreach ( $conditions as $key => $value )
		{
			...
			$condition = self::check_terms( $taxonomy, $value );
		...
		foreach ( $conditions as $key => $value )
		{
			$condition = self::check_terms( $key, $value );
	 ...
	 * @return bool
	 */
	public static function check_parent_terms( $taxonomy, $terms )
	{
		$terms = self::csv_to_array( $terms );
	 ...
	 * @return array
	 */
	private static function csv_to_array( $string )
	{
