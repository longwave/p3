...
class pipdig_theme_bloglovin_widget extends WP_Widget {
  ...
  public function __construct() {
      $widget_ops = array('classname' => 'pipdig_theme_bloglovin_widget', 'description' => __("Display your Bloglovin' follower count.", 'pipdig-textdomain') );
      $this->WP_Widget('pipdig_theme_bloglovin_widget', 'pipdig - ' . __("Bloglovin' Widget", 'pipdig-textdomain'), $widget_ops);
  }
  ...
  function widget($args, $instance) {
    ...
    if (!empty($bloglovin_url)) {
		if ($bloglovin_official) { //use official widget
			...
			if ($bloglovin_count) {
				$bloglovin_widget_output = '<p><a href="'. $bloglovin_url .'" target="blank" rel="nofollow" class="pipdig-bloglovin-widget">' . $bloglovin_icon . $bloglovin_count . ' ' . __("Followers on Bloglovin'", 'pipdig-textdomain') . '</a></p>';
			} else {
				$bloglovin_widget_output = '<p><a href="'. $bloglovin_url .'" target="blank" rel="nofollow" class="pipdig-bloglovin-widget">' . $bloglovin_icon . __("Follow on Bloglovin'", 'pipdig-textdomain') . '</a></p>';
			}
		}
		echo $bloglovin_widget_output;
	} else {
		$cust_url = admin_url( 'customize.php' );
		printf(__("Widget setup not complete. You must add your bloglovin profile to the 'Social Media Icons' section of the <a href='%s'>Customizer</a>.", 'pipdig-textdomain'), $cust_url );
	}
  ...
  public function form( $instance ) {
	...
	$cust_url = admin_url( 'customize.php' );
	?>
	<p><?php _e("This widget will display your total Bloglovin' follower count.", 'pipdig-textdomain'); ?></p>
	<p><?php printf(__("You can style this widget in the <em>Bloglovin' Widget</em> section of the <a href='%s'>Customizer</a>.", 'pipdig-textdomain'), $cust_url ); ?></p>
